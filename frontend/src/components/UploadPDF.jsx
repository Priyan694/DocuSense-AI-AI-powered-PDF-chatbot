...
export default function UploadPDF({
  ...
  return (
    <section className="overflow-hidden rounded-[34px] bg-ink p-6 text-white shadow-panel">
      <div className="absolute opacity-0" />
      <p className="text-xs font-semibold uppercase tracking-[0.24em] text-gold">DocuSense AI</p>
      <h1 className="mt-2 font-display text-4xl leading-tight">
        Upload PDFs, learn faster, and switch between three LLMs.
      </h1>
      <p className="mt-4 max-w-xl text-sm leading-6 text-white/75">
        Upload one or more PDFs, generate a rich learning summary, and ask both factual and conceptual questions with cited page references.
      </p>
      ...
      <div className="mt-5 flex flex-wrap gap-3">
        <span className="rounded-full bg-white/10 px-4 py-2 text-sm text-white/80">
          {hasSession ? "PDF session ready" : "Waiting for upload"}
        </span>
        <span className="rounded-full bg-white/10 px-4 py-2 text-sm text-white/80">
          {uploadedPdfCount ? `${uploadedPdfCount} uploaded PDF(s)` : "No files indexed"}
        </span>
        <span className="rounded-full bg-white/10 px-4 py-2 text-sm text-white/80">
          {chunkCount ? `${chunkCount} chunks` : "No chunks yet"}
        </span>
      </div>
      ...
      <div className="mt-5 flex flex-wrap gap-3">
        ...
        <button
          onClick={onReset}
          disabled={!hasSession && !files.length}
