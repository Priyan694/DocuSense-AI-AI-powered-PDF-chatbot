...
async def run_pdf_action(payload: ActionRequest) -> ActionResponse:
    query_map = {
        ...
        "compare": payload.compare_instruction or "Compare the uploaded PDF content.",
    }

    retrieved = retrieve_documents(payload.session_id, query_map[payload.action], top_k=6)
    context = "\n\n".join(
